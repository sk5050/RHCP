...
class ELEVATORModel_2011_2(object):
    ...
    def state_transitions(self, state, action):
        ...
        for px_ind in range(self.w):
            ...
            if px_pos==-2:
                ...
                if current_pos==px_pos and action=='O':
                    new_px_pos.append(-1)
                else:
                    new_px_pos.append(px_pos)
        ...
        for hidden_ind in range(self.h):
            ...
            if hidden_pos==0:
                ...
                if current_pos==hidden_pos and action=='O':
        ...
        if action=='U':
            new_pos = min(self.n, current_pos + 1)
        elif action=='D':
        ...
        new_states = [[[new_px_pos, new_hidden_pos, new_pos, new_is_open], 1.0]]
        ...
        for unarrived_hidden_ind in unarrived_hidden_ind_list:
            ...
            for new_state in new_states:
                for trans in ['NA', 'A']:
                    ...
                    if trans=='NA':
                        new_states_temp.append([[new_px_pos, new_hidden_pos_temp, new_pos, new_is_open], new_state[1]*0.25])
                    else:
                        new_hidden_pos_temp[unarrived_hidden_ind] = self.hidden_origin[unarrived_hidden_ind]
                        new_states_temp.append([[new_px_pos, new_hidden_pos_temp, new_pos, new_is_open], new_state[1]*0.75])
